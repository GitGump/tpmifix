(function() {
	 ...
	 *
	 * e.g. mod manage, local storage, login auth, network request, i18n translate...
	 *
	 * @memberof tpmifix
	 * @ngdoc overview
	 * @name tpmifix.service
	 * @requires tpmifix.protocol
	 * @requires tpmifix.directive
	 * @requires ionic
	 * @requires ngAnimate
	 * @requires ngMessages
	 ...
	 */
	angular.module('tpmifix.service', ['tpmifix.protocol', 'tpmifix.directive', 'ionic', 'ngAnimate', 'ngMessages', 'LocalStorageModule', 'toastr', 'pascalprecht.translate', 'oc.lazyLoad', 'angular-loading-bar', 'cfp.loadingBar'])
	...
	.run(['$ionicPlatform', 'serviceConstant', 'serviceValue', 'logService', 'modService',
		function($ionicPlatform, serviceConstant, serviceValue, logService, modService) {
			...
			$ionicPlatform.ready(function() {
				...
				serviceValue.serverAddress = serviceConstant.SERVER_INFO.SERVER_DNS;
				// init all mods
				modService.initMods();
			});
		}
	...
	.constant('serviceConstant', {
		...
		MOD: {
			...
			COMMON: 'common', // e.g. www/mod/common
			HOME: 'home', // e.g. www/mod/home
			LOGIN: 'login' // e.g. www/mod/login
		},
		DEFAULT_LOGIN_ACCOUNT: {
			// hard-coded username
			USERNAME: 'admin',
			// hard-coded password. it's "admin" when device is factory status.
			PASSWORD: 'admin'
		},
		SERVER_INFO: {
			SERVER_IP: '192.168.1.1',
			SERVER_DNS: 'tplogin.cn',
			AUTH_CGI: 'cgi-bin/qcmap_auth',
			WEB_CGI: 'cgi-bin/qcmap_web_cgi',
			AUTH_MODULE: 'authenticator',
			WEB_MODULE: 'webServer',
			STATUS_MODULE: 'status',
			FLOWSTAT_MODULE: 'flowstat'
		},
		AJAX_TIMEOUT: {
			SHORT: 2 * 1000, // 2s, usually for device or login detecting
		...
		// these AUTH_RESULT may be just valid for client, don't use them to judge request/response result
		AUTH_RESULT: {
			SUCCESS: 0,
			HAVE_LOGIN: 1,
			PWD_WRONG: 2,
		...
		GO_HOME_WHEN: {
			DEVICE_DISCONNECT: true, // should child mod go back home when device disconnected?
			DEVICE_DISCONNECT_MAX: 6, // Only when DEVICE_DISCONNECT reach max count, then do GO_HOME_WHEN action.
	...
	.value('serviceValue', {
		...
		serverAddress: '',
		isLogin: false,
		deviceDisconnectedCount: 0 // Refer to serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT.
	})
	...
	.provider('service', ['serviceConstant',
		function(serviceConstant) {
			/**
			 * @description
			 * Set mod info, e.g. mod path, entry file, html file, login mod...
			 *
			 * @example
			 * angular.module('tpmifix.mod.home', ['tpmifix.service'])
			...
			/**
			 * @description
			 * Set default login account (when in factory default status), e.g. username, password...
			 *
			 * @example
			 * angular.module('tpmifix.mod.home', ['tpmifix.service'])
			 * .config(['serviceProvider', function(serviceProvider) {
			...
			/**
			 * @description
			 * Set server info, e.g. device ip, cgi path, some module name...
			 *
			 * @example
			 * angular.module('tpmifix.mod.home', ['tpmifix.service'])
			 * .config(['serviceProvider', function(serviceProvider) {
			 ...
			 *         DEVICE_DISCONNECT : false,	// should child mod go back home when device disconnected?
			 *         DEVICE_DISCONNECT_MAX : 3,	// Only when DEVICE_DISCONNECT reach max count, then do GO_HOME_WHEN action.
	 ...
	 */
	.factory('logService', ['serviceConstant', '$log', '$ionicHistory',
		function(serviceConstant, $log, $ionicHistory) {
			/**
			 * @description
			 * Write a log message.
			 *
			...
			var log = function(message) {
				//$log.log('[tpmifix:logService.$log.log(' + $ionicHistory.currentStateName() + ')] ' + message);
				console.log('[tpmifix:logService.console.log(' + $ionicHistory.currentStateName() + ')] ' + message);
			}
			...
			var error = function(message) {
				//$log.error('[tpmifix:logService.$log.error(' + $ionicHistory.currentStateName() + ')] ' + message);
				console.error('[tpmifix:logService.console.error(' + $ionicHistory.currentStateName() + ')] ' + message);
			}
			...
			var warn = function(message) {
				//$log.warn('[tpmifix:logService.$log.warn(' + $ionicHistory.currentStateName() + ')] ' + message);
				console.warn('[tpmifix:logService.console.warn(' + $ionicHistory.currentStateName() + ')] ' + message);
			}
			...
			var info = function(message) {
				//$log.info('[tpmifix:logService.$log.info(' + $ionicHistory.currentStateName() + ')] ' + message);
				...
				console.log('[tpmifix:logService.console.info(' + $ionicHistory.currentStateName() + ')] ' + message);
			}
			...
			var debug = function(message) {
				//$log.debug('[tpmifix:logService.$log.debug(' + $ionicHistory.currentStateName() + ')] ' + message);
				// As cordova-plugin-console's default log level is WARN, and didn't afford APIs to change log level,
				// so we use higher console.log API to simulate console.debug
				console.log('[tpmifix:logService.console.debug(' + $ionicHistory.currentStateName() + ')] ' + message);
			}
	...
	.factory('languageService', ['$translatePartialLoader', '$translate',
		function($translatePartialLoader, $translate) {
			 ...
			 * translate('NAMESPACE.PARAGRAPH', {name:'pax', time:'2016'}, function(string) {
			 *     $scope.namespaced_paragraph = string;
			 * });
			 *
	 ...
	 */
	.factory('linkService', ['serviceConstant', 'logService', 'authInfoService', '$state', '$stateParams', '$ionicHistory', '$ionicPlatform',
		function(serviceConstant, logService, authInfoService, $state, $stateParams, $ionicHistory, $ionicPlatform) {
			var backupData = {};
			 ...
			 *
			 * This is used to make some special mod (like introduce) happy. If error occurs, won't auto jump to home mod.
			 *
			...
			var preventAutoJumpWhenError = function() {
				if (backupData.GO_HOME_WHEN) {
					...
					backupData.GO_HOME_WHEN.DEVICE_DISCONNECT = serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT;
					serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT = false;
			...
			var allowAutoJumpWhenError = function() {
				if (backupData.GO_HOME_WHEN) {
					serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT = backupData.GO_HOME_WHEN.DEVICE_DISCONNECT;
			...
			var _goBack = function(backCount) {
				...
				if (backCount && angular.isNumber(backCount) && backCount < -1) {
					...
					if (getSkipLoginCount() > 0 || authInfoService.isLogin()) {
						...
						// - If user do jump: `home -> wifi -(not login, redirect to)-> login -(cancel, back to)-> home`,
			 ...
			 * Refer to http://localhost:4000/docs/api/service/$ionicPlatform/
			 *
			 * @memberof linkService
	...
	.factory('modService', ['serviceConstant', 'serviceValue', 'httpService', 'logService', 'languageService', 'linkService', 'authInfoService', 'promptService', '$ocLazyLoad', '$rootScope', '$timeout',
		function(serviceConstant, serviceValue, httpService, logService, languageService, linkService, authInfoService, promptService, $ocLazyLoad, $rootScope, $timeout) {
			 ...
			 */
			var initMenus = function(modInfo) {
				...
				async.whilst(
					...
					function(callback) {
						getModEntry(modInfo.entry.childMods[i], function(childModInfo) {
							...
							childModInfo.entry[lang].link = childModInfo.mod;
							childModInfo.entry[lang].icon = getModImgUrl(childModInfo.mod + '.png', childModInfo.mod);
							if (childModInfo.entry.location === "sideMenu" && modInfo.entry.supportLocation.indexOf("sideMenu") > -1) {
								serviceValue.menus.sideMenus.push(childModInfo.entry[lang]);
							} else if (childModInfo.entry.location === "gridMenu" && modInfo.entry.supportLocation.indexOf("gridMenu") > -1) {
								serviceValue.menus.gridMenus.push(childModInfo.entry[lang]);
							} else if (childModInfo.entry.location === "hide" && modInfo.entry.supportLocation.indexOf("hide") > -1) {
								// "hide", don't process
								//logService.debug("modService.initMenus: mod(" + childModInfo.mod + ") wants entry(hide), needs no process.");
							} else if (childModInfo.entry.location === "home" && modInfo.entry.supportLocation.indexOf("home") > -1) {
								// "home", don't process
			...
			var initMods = function(callback) {
				getModEntry(serviceConstant.MOD.HOME, function(modInfo) {
					...
					// When language change start, do something.
					$rootScope.$on('$translateChangeStart', function() {
						// Re-Load child mod's entry, and language.
						initMenus(modInfo);
					});

					// Load common and home mod's language (file)
					languageService.loadLanguage(serviceConstant.MOD.COMMON);
					languageService.loadLanguage(modInfo.mod);

					// Load home mod's js and css (file)
					$ocLazyLoad.load(serviceConstant.MOD.PATH + "/" + modInfo.mod + "/" + serviceConstant.MOD.CSS);
					$ocLazyLoad.load(serviceConstant.MOD.PATH + "/" + modInfo.mod + "/" + serviceConstant.MOD.JS);

					// Load child mod's file (css, js, i18n...), and insert to angular module dependency.
					// e.g. js, css, html (ng-template) files...
					loadMods(modInfo.entry.childMods);
			 ...
			 * @param {object|undefined} callbacks The object contains callback functions of View LifeCycle and Events, Refer to [ionView](http://localhost:4000/docs/api/directive/ionView/).
				...
				if (callbacks) {
					...
					if (angular.isFunction(callbacks.pause)) {
						document.addEventListener('pause', function() {
							logService.debug('modService.initMod: the pause event fires.');
							callbacks.pause();
						}, false);
					}
					if (angular.isFunction(callbacks.resume)) {
						document.addEventListener('resume', function() {
							logService.debug('modService.initMod: the resume event fires.');
	...
	.factory('authService', ['serviceConstant', 'serviceValue', 'logService', 'linkService', 'serverDataService', 'authInfoService', '$window',
		function(serviceConstant, serviceValue, logService, linkService, serverDataService, authInfoService, $window) {
			...
			var login = function(password, callback) {
				...
				if (!password) {
					logService.error('authService.login: password is blank!');
				...
				serverDataService.request({
					ignoreLoadingBar: true,
					module: serviceConstant.SERVER_INFO.AUTH_MODULE,
					action: serviceConstant.AJAX_ACTION.LOAD,
					callback: function(data) {
						if (!data || (data.result !== serviceConstant.AUTH_RESULT.SUCCESS && data.result !== serviceConstant.AUTH_RESULT.HAVE_LOGIN)) {
							logService.error('authService.login: load fail!');
							authInfoService.markLogin(false);
							if (angular.isFunction(loginCallback)) {
								loginCallback(data, false);
							}
							return;
						}

						if (data.result === serviceConstant.AUTH_RESULT.HAVE_LOGIN && data.authedIP !== '0.0.0.0') {
							logService.warn('authService.login: You or someone else already log in using another device, Enforce to login will lead other be kicked out.');
						}

						var digest = CryptoJS.MD5([serviceConstant.DEFAULT_LOGIN_ACCOUNT.USERNAME, password, data.nonce].join(':')).toString();

						serverDataService.request({
							ignoreLoadingBar: true,
							module: serviceConstant.SERVER_INFO.AUTH_MODULE,
							action: serviceConstant.AJAX_ACTION.LOGIN,
							data: {
								'digest': digest
							},
							callback: function(data) {
								var isLogin = false;
								if (!data || !data.token || data.result !== serviceConstant.AUTH_RESULT.SUCCESS) {
									isLogin = false;
									logService.error('authService.login: login fail!');
								} else {
									isLogin = true;
									authInfoService.set(data.token);
								}
			...
			var logout = function(callback) {
				...
				serverDataService.request({
					ignoreLoadingBar: true,
					module: serviceConstant.SERVER_INFO.AUTH_MODULE,
					action: serviceConstant.AJAX_ACTION.CLOSE,
					token: authInfoService.get(),
					callback: function(data) {
						if (!data || data.result !== serviceConstant.AUTH_RESULT.SUCCESS) {
							isLogout = false;
							logService.warn('authService.logout: logout fail! but always continue.');
						} else {
							isLogout = true;
						}
			...
			var checkLogin = function(callback) {
				...
				serverDataService.request({
					ignoreLoadingBar: true,
					module: serviceConstant.SERVER_INFO.WEB_MODULE,
					action: serviceConstant.AJAX_ACTION.KEEP_ALIVE,
					timeout: serviceConstant.AJAX_TIMEOUT.SHORT,
					callback: function(data) {
						if (data) {
							if (data.result == serviceConstant.AUTH_RESULT.SUCCESS) {
								isLogin = true;
							} else {
								isLogin = false;
							}
						} else {
							if (authInfoService.isLogin()) {
								// Give me a chance... WTF network error
								if (serviceValue.deviceDisconnectedCount == serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT_MAX) {
									logService.warn('authService.checkLogin: judged as logout, because disconnected from device');
									isLogin = false;
								} else {
									isLogin = true;
								}
			...
			var checkAttempt = function(callback) {
				...
				serverDataService.request({
					ignoreLoadingBar: true,
					module: serviceConstant.SERVER_INFO.AUTH_MODULE,
					action: serviceConstant.AJAX_ACTION.CHECK_ATTEMPT,
					callback: function(data) {
			...
			/**
			 * @description
			 * Check if device is factory default.
			 *
			 * We will get and set a default token automatically.
			 *
			 * @memberof authService
			 * @param {function} callback The callback function, invoked with arg `{json-object} data` and `{boolean} isFactory` after check.
			 */
			var checkFactory = function(callback) {
				var isFactory = false;
				var checkCallback = callback;

				serverDataService.request({
					ignoreLoadingBar: true,
					module: serviceConstant.SERVER_INFO.STATUS_MODULE,
					action: serviceConstant.AJAX_ACTION.LOAD,
					noToken: true,
					callback: function(data) {
						if (data && data.factoryDefault) {
							isFactory = true;
						}
						if (angular.isFunction(checkCallback)) {
							checkCallback(data, isFactory);
						}
					}
				});
			}
			 ...
			 */
			var updatePassword = function(oldPassword, newPassword, callback) {
				var isUpdateSuccess = false;

				serverDataService.request({
					ignoreLoadingBar: true,
					module: serviceConstant.SERVER_INFO.AUTH_MODULE,
					action: serviceConstant.AJAX_ACTION.UPDATE,
					data: {
					...
					callback: function(data) {
						if (data && data.result == serviceConstant.AUTH_RESULT.SUCCESS) {
							...
							logService.error('authService.updatePassword: update password fail!');
						}
						if (angular.isFunction(callback)) {
			...
			/**
			 * @description
			 * If the device is factory default status, we could login using default PASSWORD (e.g. 'admin').
			 *
			 * @memberof authService
			 * @alias factory.login
			 * @param {function} callback The callback function, invoked with arg `{json-object} data` and `{boolean} isLogin` after try login.
			 */
			var factoryLogin = function(callback) {
				login(serviceConstant.DEFAULT_LOGIN_ACCOUNT.PASSWORD, callback);
			}

			/**
			 * @description
			 * If the device is factory default status, we could set new password using default PASSWORD (e.g. 'admin').
			 *
			 * @memberof authService
			 * @alias factory.setPassword
			 * @param {string} newPassword The new password you want to set.
			 * @param {function} callback The callback function, invoked with arg `{json-object} data` and `{boolean} isUpdateSuccess` after set.
			 */
			var factorySetPassword = function(newPassword, callback) {
				updatePassword(serviceConstant.DEFAULT_LOGIN_ACCOUNT.PASSWORD, newPassword, callback);
			}
			...
			return {
				...
				factory: {
					login: factoryLogin,
					setPassword: factorySetPassword
				}
			}
		}
	])
		...
		function(serviceConstant, serviceValue, httpService, logService, localDataService, authInfoService, linkService, promptService) {
			...
			var request = function(opts) {
				...
				if ((module === serviceConstant.SERVER_INFO.WEB_MODULE && (action === serviceConstant.AJAX_ACTION.KEEP_ALIVE || action === serviceConstant.AJAX_ACTION.UNSET_DEFAULT)) ||
					...
					(module !== serviceConstant.SERVER_INFO.WEB_MODULE && module !== serviceConstant.SERVER_INFO.AUTH_MODULE)) { // other modules such as status, wlan etc.
					// read token from service.auth if need
					var token = localDataService.getDevice('token');
					if (token) {
						// For checkFactory (noToken is true), remove the token even if we have it
						if (!opts.noToken) {
						...
						if (module !== serviceConstant.SERVER_INFO.STATUS_MODULE && !(module === serviceConstant.SERVER_INFO.FLOWSTAT_MODULE && action === serviceConstant.AJAX_ACTION.LOAD)) {
				...
				httpService.post(url, jsonObj, config, function(data) {
					serviceValue.deviceDisconnectedCount = 0;
					if (data.result < 0) {
					...
					if (httpServer == 'http://' + serviceConstant.SERVER_INFO.SERVER_DNS) {
					...
					serviceValue.deviceDisconnectedCount++;
					if (serviceValue.deviceDisconnectedCount == serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT_MAX) {
						logService.error('serverDataService.request: fail to connect device, reach max count=' + serviceValue.deviceDisconnectedCount);
						// reset deviceDisconnectedCount
						serviceValue.deviceDisconnectedCount = 0;
						// reset serverAddress
						serviceValue.serverAddress = serviceConstant.SERVER_INFO.SERVER_DNS;
						if (serviceConstant.GO_HOME_WHEN.DEVICE_DISCONNECT) {
							promptService.toast.warning('COMMON.CONTENT.DEVICE_DISCONNECT');
							linkService.goBackHome();
						}
					} else {
						logService.warn('serverDataService.request: fail to connect device, accumulated count=' + serviceValue.deviceDisconnectedCount);
					}
				});
			}
	...
	.factory('promptService', ['languageService', '$timeout', '$ionicPopup', '$ionicLoading', 'toastr', 'cfpLoadingBar',
		function(languageService, $timeout, $ionicPopup, $ionicLoading, toastr, cfpLoadingBar) {
			 ...
			 * @param {object} opts The options for showing popup. Refer to [$ionicPopup](http://localhost:4000/docs/api/service/$ionicPopup/).
			 ...
			 * @param {object} opts The options for showing the alert. Refer to [$ionicPopup](http://localhost:4000/docs/api/service/$ionicPopup/).
			 ...
			 * @param {object} opts The options for showing the confirm. Refer to [$ionicPopup](http://localhost:4000/docs/api/service/$ionicPopup/).
			 ...
			 * @param {object} opts The options for showing the prompt. Refer to [$ionicPopup](http://localhost:4000/docs/api/service/$ionicPopup/).
			 ...
			 * @param {object} opts The options for the loading indicator. Refer to [$ionicLoading](http://localhost:4000/docs/api/service/$ionicLoading/).
			 */
			...
			var loadingCloseIcon = '<i class="ion-close-circled assertive loading-closeIcon" ng-click="tpService.promptService.loading.hide(0)"></i>';
	 ...
	 * @requires $window
	 */
	.factory('appUpdateService', ['serviceValue', 'httpService', 'logService', 'protocolConstant', 'diffUtil', '$window',
		function(serviceValue, httpService, logService, protocolConstant, diffUtil, $window) {
			/**
			 * @description
			...
			var appStore = {
				ANDROID: {
					APP_PACKAGE_NAME: 'com.tplink.tpmifix', // TODO: This is a test name, replace it with true value.
					LOOKUP_URL: 'https://wap.tplinkcloud.com.cn',
					CHECK_NEW_VER: 'getNewestAppVersion',
					APP_URL: '' // not a constant on tplinkcloud
				},
				IOS: {
					LOOKUP_URL: 'http://itunes.apple.com/lookup?id=',
					APP_URL: 'https://itunes.apple.com/app/id',
					APP_ID: '999189880' // TODO: This is the "waixiao" app's id, replace it with "neixiao".
				}
			};
	 ...
	 * Other module can depend on `tpService` and use any API under it, like `tpService.xxx.yyy()`.
	 * Html can use `tpService.xxx.yyy()` directly as it has been exported to `$rootScope`.
	 *
	 * @example
	 ...
	 */
	.factory('tpService', ['$rootScope', '$window', 'serviceConstant', 'serviceValue', 'logService', 'languageService', 'linkService', 'httpService', 'modService', 'authInfoService', 'authService',
		...
		function($rootScope, $window, serviceConstant, serviceValue, logService, languageService, linkService, httpService, modService, authInfoService, authService,
			...
			$rootScope.tpService = api;
			// Export browser window
			$window.tpService = api;
			return api;
		}
	])
